...
def test_beyazit(input_file_name):
    ...
    input_file_content = map(lambda x: x[:-1].split(", "), input_file.readlines())
    input_file.close()
    tasks = "const task_t Tasks[N] = {"
    dependencies = "bool dependencies[N][N] = {"
    hyperperiod = 1
    periods = []
    offset = 0
    ...
    xml_file_name = "/RTS/cluster/work/beyazit/SPORADIC_efficient_new_model_experiments/scripts/temp_models/" + input_file_name.split("inputs/")[1].split(".csv")[0] + ".xml"
    xml_file = open(xml_file_name, "w+")
    xml_file.writelines(xml_file_content)
    xml_file.close()
    q_file_name = "/RTS/cluster/work/beyazit/SPORADIC_efficient_new_model_experiments/scripts/temp_models/" + input_file_name.split("inputs/")[1].split(".csv")[0] + ".q"
    q_file = open(q_file_name, "w+")
    q_file.write(q_file_content)
    q_file.close()
    uppaal_verifier.verify_xml_file(N, xml_file_name, q_file_name, min_period, max_period, str(number_of_jobs), input_file_name, "4h")
if len(sys.argv) > 1:
